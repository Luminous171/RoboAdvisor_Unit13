...
def validate_slots(age, investmentAmount, intent_request):
    
    if age is not None:
        age = parse_int(age)
        if age <= 0:
            return build_validation_result(False, "age", "You cannot be zero years old or less until time travel exists.")
        elif age >= 65:
            return build_validation_result(False, "age", "You should already be retired, too late to save now.")
    if investmentAmount is not None:
        investmentAmount = parse_int(investmentAmount)
        if investmentAmount < 5000:
            return build_validation_result(False, "investmentAmount", "You are too poor to invest.")
...
def get_recommendation(risk_level):
    if risk_level == "None":
        ...
        initial_recommendation = "You need a risk tolerance before getting a recommendation."
...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        validation_result = validate_slots(age, investment_amount, intent_request)
